...
-- 图书数据
INSERT IGNORE INTO book (isbn, title, author, publisher, publish_year, category, category_name, language, pages, price, summary, status, total_copies, available_copies, location, shelf_no, deleted, create_time, update_time) VALUES
...
-- 阅览室数据
INSERT IGNORE INTO reading_room (name, location, capacity, total_seats, available_seats, open_time, close_time, description, status, deleted, create_time, update_time) VALUES
...
-- 座位数据(为每个阅览室创建部分座位)
INSERT IGNORE INTO seat (room_id, seat_number, row_num, col_num, seat_type, has_power, has_lamp, status, deleted, create_time)
...
-- 借阅记录数据
INSERT IGNORE INTO borrow_record (user_id, book_id, borrow_date, due_date, status, renew_count, deleted, create_time, update_time)
SELECT 
...
-- 预约记录数据
INSERT IGNORE INTO reservation (user_id, book_id, reserve_date, expire_date, status, deleted, create_time)
SELECT 
...
-- 通知数据
INSERT IGNORE INTO notification (user_id, title, content, type, is_read, deleted, create_time)
SELECT 
    (SELECT id FROM user_account WHERE username = 'student001'),
    '借阅成功通知',
...
-- 图书评价数据
INSERT IGNORE INTO book_review (book_id, user_id, rating, content, status, create_time, update_time)
SELECT 
    (SELECT id FROM book WHERE isbn = '9787020012345'),
    (SELECT id FROM user_account WHERE username = 'student001'),
    5,
...
-- 用户收藏数据
INSERT IGNORE INTO favorite (user_id, book_id, create_time)
SELECT 
    (SELECT id FROM user_account WHERE username = 'student001'),
    (SELECT id FROM book WHERE isbn = '9787020012345'),
    NOW();
...
-- 用户行为数据
INSERT IGNORE INTO user_behavior (user_id, book_id, action_type, action_detail, ip_address, create_time)
SELECT 
    (SELECT id FROM user_account WHERE username = 'student001'),
    (SELECT id FROM book WHERE isbn = '9787020012345'),
    'VIEW',
...
-- 咨询记录数据
INSERT IGNORE INTO inquiry (user_id, title, content, category, status, reply_content, replier_id, reply_date, deleted, create_time, update_time)
SELECT 
    (SELECT id FROM user_account WHERE username = 'student001'),
    '如何续借图书？',
...
-- 采购申请数据
INSERT IGNORE INTO purchase_request (requester_id, book_title, author, isbn, publisher, reason, quantity, status, deleted, create_time, update_time)
SELECT 
